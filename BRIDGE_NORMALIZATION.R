...
cat("\n\n\n\n###############################################################################################\n",
    ...
    #                 "/Users/nikhilbhagwat/Desktop/0_DATA/0.4_Biofluids/0.4.3_Olink/0.4.3.1_Simrika_Onedrive/Second_project_AD_HC/Sample_all_2ndproject.csv",
    #                 "/Users/nikhilbhagwat/Desktop/0_DATA/0.4_Biofluids/0.4.3_Olink/0.4.3.1_Simrika_Onedrive/Third_project_Genetic_cases/Sample_template_3rd_olink.csv")
...
plateB_manifest_df <- read.func.csv(filepaths_df, 4, olink=FALSE)
plateC_manifest_df <- read.func.csv(filepaths_df, 5, olink=FALSE)
...
plateC_manifest_df$SampleID <- as.character(plateC_manifest_df$SampleID)
    ...
    # #DEFENSIVE CODING" REVIEW POTENTIAL ERROR
    # if (sum(plateB_manifest_df$SampleID %in% all_template_df$SampleID) !=0) {
    #     cat("There is an issue with the IDs in the manifest of plate B: seems that some are not in the manifest for the overall dataset \n")
    #     print(subset(plateB_manifest_df, plateB_manifest_df$SampleID %in% all_template_df$SampleID ==FALSE)[, c("SampleID", "Freezer_ID")])
    # }
    # if (sum(plateC_manifest_df$SampleID %in% all_template_df$SampleID) !=0) {
    #     cat("There is an issue with the IDs in the manifest of plate C: seems that some are not in the manifest for the overall dataset \n")
    #     print(subset(plateC_manifest_df, plateC_manifest_df$SampleID %in% all_template_df$SampleID ==FALSE)[, c("SampleID", "ID")])
    # }
...
# cat("\n\n\n\n###############################################################################################\n",
#             "3. MERGING WITH DEMOGRAPHICS TO HAVE THE FREEZER IDs (TARTAGLIA ID)\n",
#             "###############################################################################################\n\n\n")

# #Resources
# # https://cran.r-project.org/web/packages/OlinkAnalyze/vignettes/bridging_introduction.html


# #Steps prior to current analysis: 
# ##Design: olink_bridgeselector can be used in first plate to identify good bridging samples that should then be added on Run 2, etc. 
#         #olink_pca_plot cn be used to check this was the right choice


# # data.frame(SampleID = intersect(plateA_df$SampleID, plateC_df$SampleID)) %>%
